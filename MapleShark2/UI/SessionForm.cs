...
namespace MapleShark2.UI
{
    public partial class SessionForm : DockContent
    {
        ...
        private Results ProcessPacket_TR(byte[] bytes, bool isOutbound, DateTime timestamp)
        {
            if (mTerminated) return Results.Terminated;

            try
            {
                ByteReader packet = new ByteReader(bytes);
            ...
            {
                logger.Fatal(ex, "Exception while processing packets 2");
                return Results.CloseMe;
            }
        }

        private Results ProcessPacket_MS2(byte[] bytes, bool isOutbound, DateTime timestamp)
        {
            if (mTerminated) return Results.Terminated;
            ...
            if (Build == 0)
            {
                ...
                if (opcode != 0x01)
                {
                    // RequestVersion
                    logger.Debug(
                        $"Connection on port {mLocalEndpoint} did not have a valid MapleStory2 Connection Header");
                    return Results.CloseMe;
                }
